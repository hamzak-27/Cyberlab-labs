...
class VMProvisionerService {
   ...
   * Supports both NAT (legacy) and VPN bridge networking
   * @private
   */
  async allocateNetworkPorts(sessionId, options = {}) {
    const { useVPN = false, userSubnet = null } = options;
    
    if (useVPN && userSubnet) {
      // VPN Bridge mode - assign VM IP within user's subnet
      const vmIP = `10.10.${userSubnet.subnetNumber}.10`;
      ...
      // NAT mode with port forwarding (legacy)
  ...
  /**
   * Configure VM networking - supports both NAT and VPN bridge modes
   * @private
   */
    ...
    if (networkConfig.mode === 'bridge') {
      ...
      return {
        ...
        ssh: {
          host: ipAddress,
          port: sshPort,
          command: `ssh user@${ipAddress} -p ${sshPort}`,
